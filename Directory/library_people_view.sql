...
select 
	...
	from (((`publications`.`incoming_people` left join `publications`.`library_data` on((`publications`.`library_data`.`nuid` = `publications`.`incoming_people`.`UNLUNCWID`))) 
	...
	where (`publications`.`incoming_people`.`UNLHRPRIMARYDEPARTMENT` = 'University Libraries') 
	group by `publications`.`incoming_people`.`SN`,`publications`.`incoming_people`.`GIVENNAME`
