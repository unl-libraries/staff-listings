		...
		elseif ($this->request->is('post') && !empty($this->request->data['User']['email'])){
			...
			if (isset($this->request->data['User']['token']) && ($this->request->data['User']['token'] !='')){
				...
				if (($db_token!='' && $db_token != NULL) && ($this->request->data['User']['token'] == $db_token)){
					...
					$this->User->save(array('id'=>$user_id,'token'=>null));
					
					//allow the password reset form					
					$user_id = $this->User->field('id',array('email'=>$email));
					$this->Auth->login($this->User->findById($user_id));
			...
			if (!empty($user)){
				...
				$message .= "If you requested this information and either do not have a password hint, or the hint does not help you remember your password, you can reset your password using this link $url. (Link won't work yet, still under construction!) "."\r\n \r\n";
